...
task :post_post_to_bluesky => :environment do
  puts "posting post articles"
  
  # We set up the authentication keys.
  bluesky_handle = ENV['POST_BLUESKY_HANDLE']
  bluesky_app_password = ENV['POST_BLUESKY_APP_PASSWORD']
  
  # We find the publisher.
  publisher = Publisher.find_by_name( 'POST' )
  
  # We find all POST articles not posted to Blueky.
  articles = Article.where( "publisher_id =?", publisher.id ).where( 'is_posted_to_bluesky IS FALSE' )
  puts "Posting #{articles.size} POST articles to Bluesky"
  ...
  articles.each do |article|
    ...
    facets = create_facets_post( post_text )
    ...
    if response.code == '200'
      ...
      # ... we report success.
      puts 'Posting POST briefing to Bluesky - success'
    ...
    # Otherwise, if the request is not successful ...
      ...
      # ... we report failure.
      puts "Posting POST briefing to Bluesky - failure with response code #{response.code}"
    end
